...
name_2013 <- names_2013[names_2013$prop == max(names_2013$prop), "name"]
...
most_popular_in_year <- function(year) {
  names <- names[names$year == year, ]
  most_popular <- names_year[names$prop == max(names$prop), "name"]
  most_popular
}

# What was the most popular female name in 1994?
name_1994 <- most_popular_in_year(1994)
...
number_in_million <- function(name, year) {
  pop <- names[names$year == year & names$name == name, "prop"]
}
