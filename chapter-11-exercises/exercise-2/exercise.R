...
length(unique(make$make))


# Filter the data set for vehicles manufactured in 1997
cars_from_1997 <- filter(vehicles, year == 1997)


# Arrange the 1997 cars by highway (`hwy`) gas milage
cars_from_1997 <- arrange(cars_from_1997, hwy)


# Mutate the 1997 cars data frame to add a column `average` that has the average
# gas milage (between city and highway mpg) for each car
cars_from_1997 <- mutate(cars_from_1997, average = ((hwy + cty) / 2))
...
filtered <- filter(two_wheel_vehicles, hwy == min(hwy))
...
make_year_filter <- function(make_choice, year_choice) {
  filtered_1 <- filter(vehicles, make == make, year == year_choice)
  filtered_1 <- filter(filtered_1, hwy == max(hwy))
  selected_done <- select(filtered_1, model)
  selected_done
}
